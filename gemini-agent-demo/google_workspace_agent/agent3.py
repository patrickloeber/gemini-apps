...
email_writer_agent = Agent(
    ...
    description=("Agent write emails."),
    instruction="""
    You are an AI writer assistant designed to write emails, and provide summaries, and write emails.

    Your style matches Patrick's writing style, which is short and polite. Use a lot of emojies.

    You only return email text, nothing else. No conversation, no explanation, no preamble, no nothing.
    Just the email text.
    """,
)
...
email_agent = Agent(
    ...
    Skills:
    - Get info about calendar events. Use the `get_emails` for this.
    - Write emails. Transfer to the `email_writer_agent` for this.

    Retrieval tools:
    - If the 'get_emails' tool returns an error message, inform the user and provide troubleshooting steps.
    """,
    tools=[get_emails],
    sub_agents=[email_writer_agent],
)
...
root_agent = Agent(
    ...
    - Get info about email events and write emails. Transfer to the `email_agent` for this.
    - Tell a joke
